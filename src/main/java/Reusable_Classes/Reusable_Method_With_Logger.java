...
public class Reusable_Method_With_Logger {
    ...
    //Method to select size from the express website//created fpr ActionItem6
    public static void selectSize(WebDriver driver, String size, String elementName, ExtentTest logger){
        WebDriverWait wait = new WebDriverWait(driver,15);
        try{
            System.out.println("Choosing a " + elementName);
            WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//*[text()='" + size +"']")));
            element.click();
            logger.log(LogStatus.PASS,"Successfully selected a size");
        }catch(Exception err){
            System.out.println("Unable to select a size");
            logger.log(LogStatus.FAIL,"Unable to select a size" + err);
            getScreenShot(driver,elementName,logger);
        }//end of try catch
    }//end of selectSize
