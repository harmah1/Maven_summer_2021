...
public class Reusable_Methods {
    ...
    public static String getText(WebDriver driver, String xpath, String elementName) {
        ...
        try {
            System.out.println("Capture Text " + elementName);
            WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(xpath)));
            text = element.getText();

        } catch (Exception err) {

            System.out.println(("Unable to get text on " + elementName));
        }
        return text;
...
//get text by index
    ...
    public static void sliderSendKeysMethod(WebDriver driver, String xpath, int resetPoint, String desiredPoint, String elementName) throws InterruptedException {
        ...
        try {
            ...
            //desired loop
            //converting String desirePoint to integer so I can use it on for loop
           int intDesirePoint = Integer.parseInt(desiredPoint);
            for (int i = 0; i < intDesirePoint; i++) {
